...
const products = [
  {
    ...
    entry: { scale: 0.6, rotate: -15, opacity: 0, transition: { duration: 1.2, type: 'spring', bounce: 0.45 } },
    bg: '/images/c6.png',
    bgClass: 'rotate-12',
  },
  {
    ...
    entry: { scale: 0.6, rotate: 10, opacity: 0, transition: { duration: 1.2, type: 'spring', bounce: 0.45 } },
    bg: '/images/nut2.png',
    bgClass: 'rotate-180',
  },
  {
...
const NewProducts = () => {
  ...
  // Hover states for background image scaling
  const [hoverIdx, setHoverIdx] = useState(-1);

  return (
    <div className='w-full h-full overflow-x-hidden relative' style={{ background: 'linear-gradient(180deg,#f55d81  0%, #164863 35%, #32c0c2 65%, #0a2233 100%)' }}>
      {/* Animated heading */}
      <motion.h1
        ref={h1Ref}
      ...
      <img className='w-full h-120 opacity-95 rotate-180 '  src="/images/svg2.png" alt="" />
      <div className='absolute top-100 md:top-65 xl:top-85 2xl:top-110 flex md:flex-row flex-col justify-center w-full gap-8 mt-12'>
        {products.map((product, idx) => (
          <div key={product.name} className='relative flex items-center justify-center'>
            <img
              src={product.bg}
              alt="Decorative Background"
              className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[220px] md:w-[320px] max-w-[90vw] z-0 ${product.bgClass} pointer-events-none select-none transition-transform duration-500 ${hoverIdx === idx ? 'scale-100' : 'scale-0'}`}
            />
            <ThreeDImage
              src={product.image}
              alt={product.name}
