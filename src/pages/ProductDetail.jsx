...
const ProductDetail = () => {
  ...
  const { selectedProduct, productDetails } = useSelector(
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-6 md:p-10">
          ...
          <div className="flex  text-center md:text-left flex-col justify-center gap-6 p-4 md:p-6">
            ...
            <div className="space-y-3">
              ...
              >
                <span className="text-lg text-center text-[#164863]/60 line-through">
                  ...
                  {(productDetails && productDetails.originalPrice) ||
                    ...
                    Math.round(selectedProduct.price * 1.2)}
                  .00
                </span>
              ...
              >
                <span className="text-base text-[#164863] font-semibold tracking-wide relative after:content-[''] after:absolute after:w-full after:h-[2px] after:bg-gradient-to-r after:from-[#32c0c2] after:to-[#f55d81] after:bottom-[-4px] after:left-0 after:rounded-full">
                  Quantity
                </span>
                ...
                <div className="flex items-center rounded-xl overflow-hidden bg-gradient-to-r shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-[#e0e7ff] p-1">
                  ...
                  >
                    <span className="text-center font-bold text-lg bg-gradient-to-r from-[#164863] to-[#427D9D] bg-clip-text text-transparent">
                      {quantity}
                    </span>
                  </motion.div>
            ...
            >
              ...
              <motion.button
                whileHover={{
                  scale: 1.03,
                  boxShadow: "0 10px 25px -5px rgba(245, 93, 129, 0.4)",
                }}
                whileTap={{ scale: 0.97 }}
            ...
            >
              <h3 className="text-[#164863] text-left !p-5 font-bold text-lg mb-2">
                {(productDetails && productDetails.desc) ||
                  selectedProduct.desc ||
                  `Delicious ${selectedProduct.name} - A premium treat for chocolate lovers!`}
              </h3>
              <ul className="list-disc !ml-5 text-[#164863]/90 text-left !p-5 text-base space-y-2">
                {productDetails && productDetails.details ? (
                  productDetails.details.map((d, i) => <li key={i}>{d}</li>)
                ) : selectedProduct.details ? (
                  selectedProduct.details.map((d, i) => <li key={i}>{d}</li>)
                ) : selectedProduct && selectedProduct.accordionData ? (
                  selectedProduct.accordionData.map((item, idx) => (
                    <div
                      key={idx}
                      className={`border-b border-[#164863]/20 ${
                        idx === selectedProduct.accordionData.length - 1
                          ? "border-b-0"
                          : ""
                      }`}
                    >
                      ...
                      >
                        <div className="p-4 pt-0 text-[#164863]/80 text-sm">
                          {item.content}
                        </div>
                      </motion.div>
                    </div>
                  ...
                  <>
                    <li>
                      Made with premium ingredients for exceptional taste and
                      quality.
                    </li>
                    <li>
                      Perfect balance of flavors that will delight your taste
                      buds.
                    </li>
                    <li>
                      Carefully crafted to ensure the best experience with every
                      bite.
                    </li>
                  </>
                )}
              ...
              {productDetails && productDetails.accordionData
                ? productDetails.accordionData.map((item, idx) => (
                    <div
                      key={idx}
                      className={`border-b border-[#164863]/20 ${
                        idx === productDetails.accordionData.length - 1
                          ? "border-b-0"
                          : ""
                      }`}
                    >
                ...
                : [
                    {
                      ...
                      content:
                        "Premium quality product with excellent taste and texture.",
                    },
                    {
                      title: "Shipping Information",
                      content:
                        "Free shipping across India. Delivery within 3-5 business days.",
                    },
                    {
                      title: "Return Policy",
                      content:
                        "Easy returns within 7 days of delivery if you're not satisfied with the product.",
                    },
                  ].map((item, idx) => (
                    <div
                      key={idx}
